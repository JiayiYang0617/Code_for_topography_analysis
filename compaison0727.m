...
%No data for pans in the DEM at first in Norton Creek
ScoltHeadIsland=Zdata_ScoltHeadIsland.maskedDEM;
%show the pans, change the elevation higher
%NortonCreek(Zdata_nortoncreek.setpans>0)=0.600;
ScoltHeadIsland=sort(reshape(ScoltHeadIsland,1,numel(ScoltHeadIsland)),'descend');
...
%normalize
BWSteepleBay=(BWSteepleBay-min(BWSteepleBay))./(max(BWSteepleBay)-min(BWSteepleBay));
BWSalcott=(BWSalcott-min(BWSalcott))./(max(BWSalcott)-min(BWSalcott));
BWToellsbury=(BWToellsbury-min(BWToellsbury))./(max(BWToellsbury)-min(BWToellsbury));
Colne=(Colne-min(Colne))./(max(Colne)-min(Colne));
Deben=(Deben-min(Deben))./(max(Deben)-min(Deben));
ScoltHeadIsland=(ScoltHeadIsland-min(ScoltHeadIsland))./(max(ScoltHeadIsland)-min(ScoltHeadIsland));
MorecambeBay=(MorecambeBay-min(MorecambeBay))./(max(MorecambeBay)-min(MorecambeBay));
...
% 'Integral i=',num2str(i),'number of classes = ',num2str(elements1)
xlabel('Cumulative area [%]');
ylim([0 1]);
ylabel({'h/H'; 'h:elevation of sea level'; 'H:elevation of HAT'}); %（change it）h/H h-elevation of MSL/H-HAT
legend({'BWSteeple Bay','BWSalcott','BWTollesbury','Colne','Deben',...
    'Scolt Head Island', 'Morecambe Bay','Ribble'},'Location','southwest')
%% area precentage and mean dem talbe
